...
⚠️ **展示规则：必须严格按照下方模板格式展示参数，不得自创格式！**
⚠️ **单位说明：Gas1_pp/Gas2_pp 是百分比(%)，不是压力值(Pa)。展示时必须带%号！**
...
处理流程:
1. 获取用户修改后的新温度值 T_new
2. 使用 §3.1 的公式 S = (a × T^b) / 96485 重新计算每种气体的熵值
3. 更新 input.json 中的 Gas1_S 和 Gas2_S
4. 展示更新后的参数给用户（包括新的熵值）

**示例**: 温度从500K改为800K
- CO: S(500K) = 0.002200 eV/K → S(800K) = 0.002357 eV/K ✅ 必须更新
- O2: S(500K) = 0.002285 eV/K → S(800K) = 0.002446 eV/K ✅ 必须更新

**验证**: 修改后的熵值必须与§3.1公式计算结果一致。
